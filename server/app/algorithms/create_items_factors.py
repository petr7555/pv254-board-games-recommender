...
def train_val_test_split(df: pd.DataFrame, stratify_col_name: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ratios = [0.8, 0.1, 0.1]
    assert sum(ratios) == 1

    train, val_test = train_test_split(df, train_size=ratios[0], test_size=ratios[1] + ratios[2],
                                       stratify=df[[stratify_col_name]])

    val_size = ratios[1] / (ratios[1] + ratios[2])
    val, test = train_test_split(val_test, train_size=val_size, test_size=1 - val_size,
                                 stratify=val_test[[stratify_col_name]])

    return train, val, test


def initialize_matrix(unique_elements: np.ndarray, col_prefix: str, k_latent_factors: int,
                      upper_range: int) -> pd.DataFrame:
    np.random.seed(0)
...
def fit(x_train: pd.DataFrame, x_val: pd.DataFrame, unique_users: np.ndarray, unique_items: np.ndarray, *,
        k_latent_factors: int, n_epochs: int, learning_rate: float, reg: float,
        batch_size: int, epoch_improvement_threshold: float) -> tuple[pd.DataFrame, pd.DataFrame]:
